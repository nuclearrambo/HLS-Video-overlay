...
static int StartTransfer(XAxiVdma *InstancePtr, int DeviceId);
/*****************************************************************************/
/**
*
		...
		int vsize, int buf_base_addr, int number_frame_count, int enable_frm_cnt_intr)
{
	...
	if(vdma_context[DeviceId].init_done ==0)
	{
		...
		vdma_context->InstancePtr->UseFsync = 0;
		//User code
//		if(DeviceId == 1){
//			vdma_context[DeviceId].buffer_address = 0x85000000;
//		}
//		else {
...
//	}
	...
	if (Status != XST_SUCCESS) {
		xil_printf(
				"VDMA %d: Read channel setup failed %d\r\n", DeviceId, Status);
		if(Status == XST_VDMA_MISMATCH_ERROR)
			xil_printf("DMA Mismatch Error\r\n");
...
/* The the frame counter interrupt is enable, setting VDMA for same */
	 ...
	 */
	Status = StartTransfer(vdma_context[DeviceId].InstancePtr, DeviceId);
	if (Status != XST_SUCCESS) {
		if(Status == XST_VDMA_MISMATCH_ERROR)
			xil_printf("DMA Mismatch Error\r\n");
		return XST_FAILURE;
